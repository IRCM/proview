...
public class EnrichmentViewPresenterTest {
  ...
  private void setFields() {
    final ListDataProvider<TreatedSample> treatments = dataProvider(design.enrichments);
    int count = 0;
  ...
  public void enrichments() {
    ...
    assertEquals(DOWN, design.enrichments.getColumns().get(3).getId());
    assertEquals(resources.message(DOWN), design.enrichments.getColumn(DOWN).getCaption());
    assertTrue(containsInstanceOf(design.enrichments.getColumn(DOWN).getExtensions(),
        ComponentRenderer.class));
    assertFalse(design.enrichments.getColumn(DOWN).isSortable());
    for (TreatedSample ts : treatments.getItems()) {
      Button button = (Button) design.enrichments.getColumn(DOWN).getValueProvider().apply(ts);
  ...
  public void down() {
    ...
    Button button = (Button) design.enrichments.getColumn(DOWN).getValueProvider().apply(firstTs);
  ...
  public void down_OrderedBySampleDesc() {
    ...
    field.setValue(comment);
    Button button =
        (Button) design.enrichments.getColumn(DOWN).getValueProvider().apply(treatments.get(4));
  ...
  public void down_OrderedByContainerDesc() {
    ...
    field.setValue(comment);
    Button button =
        (Button) design.enrichments.getColumn(DOWN).getValueProvider().apply(treatments.get(5));
  ...
  public void save() {
    ...
    assertEquals(containers.size(), enrichment.getTreatedSamples().size());
    for (int i = 0; i < containers.size(); i++) {
      SampleContainer container = containers.get(i);
      TreatedSample enriched = enrichment.getTreatedSamples().get(i);
  ...
  public void save_Update() {
    ...
    assertEquals(enrichment.getTreatedSamples().size(), savedEnrichment.getTreatedSamples().size());
    for (int i = 0; i < enrichment.getTreatedSamples().size(); i++) {
      TreatedSample original = enrichment.getTreatedSamples().get(i);
      TreatedSample enriched = savedEnrichment.getTreatedSamples().get(i);
    ...
    verify(view).showTrayNotification(resources.message(SAVED, enrichment.getTreatedSamples()
  ...
  public void remove() {
    ...
    Enrichment savedEnrichment = enrichmentCaptor.getValue();
    assertEquals((Long) 7L, savedEnrichment.getId());
    verify(view).showTrayNotification(resources.message(REMOVED, enrichment.getTreatedSamples()
        .stream().map(ts -> ts.getSample().getId()).distinct().count()));
    verify(view).navigateTo(EnrichmentView.VIEW_NAME, "7");
  }
  ...
  public void enter() {
    ...
    List<TreatedSample> tss = new ArrayList<>(dataProvider(design.enrichments).getItems());
    assertEquals(containers.size(), tss.size());
    for (int i = 0; i < containers.size(); i++) {
      SampleContainer container = containers.get(i);
      TreatedSample enriched = tss.get(i);
      assertEquals(container.getSample(), enriched.getSample());
      assertEquals(container, enriched.getContainer());
    }
  ...
  public void enter_Enrichment() {
    ...
    List<TreatedSample> tss = new ArrayList<>(dataProvider(design.enrichments).getItems());
    assertEquals(enrichment.getTreatedSamples().size(), tss.size());
    for (int i = 0; i < enrichment.getTreatedSamples().size(); i++) {
      assertEquals(enrichment.getTreatedSamples().get(i), tss.get(i));
    }
    for (TreatedSample ts : tss) {
      TextField field =
          (TextField) design.enrichments.getColumn(COMMENT).getValueProvider().apply(ts);
  ...
  public void enter_EnrichmentNotId() {
    ...
    ListDataProvider<TreatedSample> tss = dataProvider(design.enrichments);
