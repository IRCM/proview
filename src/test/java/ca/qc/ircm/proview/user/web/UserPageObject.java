...
public abstract class UserPageObject extends AbstractTestBenchTestCase {
  ...
  protected PhoneNumberType getPhoneType() {
    String value = phoneTypeField().getValue();
    return Arrays.asList(PhoneNumberType.values()).stream()
        .filter(type -> type.getLabel(currentLocale()).equals(value)).findFirst().orElse(null);
  }
