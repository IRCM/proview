...
public class InstantFilterComponentPresenterTest {
  ...
  private Instant fromDate = toInstant(LocalDate.of(2016, 5, 11));
  private Instant toDate = toInstant(LocalDate.of(2016, 9, 20));
  ...
  public void beforeTest() {
    presenter = new InstantFilterComponentPresenter();
  ...
  private Instant toInstant(LocalDate localDate) {
    return localDate.atTime(0, 0).atZone(ZoneId.systemDefault()).toInstant();
  }
  ...
  private String intervalCaption(Instant fromDate, Instant toDate) {
    ...
    if (toDate == null) {
      return resources.message(RANGE_ONLY_FROM, formatter.format(toLocalDate(fromDate)));
    } else if (fromDate == null) {
      return resources.message(RANGE_ONLY_TO, formatter.format(toLocalDate(toDate)));
    } else {
      return resources.message(RANGE, formatter.format(toLocalDate(fromDate)),
          formatter.format(toLocalDate(toDate)));
    }
  }

  private void setFields() {
    view.from.setValue(toLocalDate(fromDate));
    view.to.setValue(toLocalDate(toDate));
  }
  ...
  public void set_FromEqualsTo() {
    view.from.setValue(toLocalDate(fromDate));
    view.to.setValue(toLocalDate(fromDate));
  ...
  public void set_ToBeforeFrom() {
    ...
    assertEquals(intervalCaption(null, toInstant(toDate)), view.filter.getCaption());
    verify(view).fireSaveEvent(Range.atMost(toInstant(toDate)));
  }
  ...
  public void setRange() {
    ...
    assertTrue(presenter.getRange().hasUpperBound());
    assertEquals(toDate, presenter.getRange().upperEndpoint());
    assertEquals(fromDate, toInstant(view.from.getValue()));
    assertEquals(toDate, toInstant(view.to.getValue()));
    assertEquals(intervalCaption(), view.filter.getCaption());
