...
ALTER TABLE samplecontainer
ADD CONSTRAINT IF NOT EXISTS samplecontainer_ibfk_3 FOREIGN KEY (treatmentSampleId) REFERENCES treatmentsample (id) ON DELETE SET NULL ON UPDATE CASCADE;
...
CREATE TABLE IF NOT EXISTS msanalysisverification (
  ...
  PRIMARY KEY (id),
  CONSTRAINT msanalysisverification_ibfk_1 FOREIGN KEY (msAnalysisId) REFERENCES msanalysis (id) ON DELETE CASCADE ON UPDATE CASCADE
);
