...
public class AuthenticationServiceDefault implements AuthenticationService {
  ...
  private final Logger logger = LoggerFactory.getLogger(AuthenticationServiceDefault.class);
  @PersistenceContext
  ...
  public AuthenticationServiceDefault(EntityManager entityManager,
  ...
  public void runAs(User user) {
    ...
    getSubject().checkRole(UserRole.PROTEOMIC.name());
    user = getUser(user.getId());
    if (user.isProteomic()) {
      throw new UnauthorizedException("Cannot run as a proteomic user");
    }
  ...
  private boolean credentialMatches(UsernamePasswordToken token, User user) {
    ...
    for (PasswordVersion passwordVersion : applicationConfiguration.getPasswordVersions()) {
  ...
  private Set<String> selectRoles(User user, boolean manager) {
    ...
    if (user.isProteomic()) {
      roles.add(UserRole.PROTEOMIC.name());
    }
