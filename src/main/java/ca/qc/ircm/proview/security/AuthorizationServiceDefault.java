...
@Service
@Transactional
public class AuthorizationServiceDefault implements AuthorizationService {
  private static final String PROTEOMIC = UserRole.PROTEOMIC.name();
  ...
  protected AuthorizationServiceDefault() {
  }

  protected AuthorizationServiceDefault(EntityManager entityManager,
  ...
  public boolean hasProteomicRole() {
    return getSubject().hasRole(PROTEOMIC);
  }
  ...
  public boolean hasManagerRole(User user) {
    ...
    AuthorizationInfo authorizationInfo = authenticationService.getAuthorizationInfo(
        new SimplePrincipalCollection(user.getId(), applicationConfiguration.getRealmName()));
  ...
  public void checkProteomicRole() {
    getSubject().checkRole(PROTEOMIC);
  }

  @Override
  public void checkProteomicManagerRole() {
    getSubject().checkRole(PROTEOMIC);
    getSubject().checkRole(MANAGER);
  }
